...
for perc=1:100
    waitbar(perc/100,f,sprintf("Processing %d of 100.",perc));
    ...
    parfor i=1:nKFolds
        ...
        % Regression
        %=========================
        regressionModel=fitlm(training_features,training_target);
        
        predictions_train=predict(regressionModel,training_features);
        predictions_test=predict(regressionModel,testing_features);
        ...
        RSquared=[RSquared;[(1-(regressionModel.SSE/regressionModel.SST))]];
    end
    
    RMSE_decisiontree_train=mean(RMSE_Train);
    RMSE_decisiontree_test=mean(RMSE_Test);

    MSE_Values(end+1,:)=[perc,RMSE_decisiontree_train,RMSE_decisiontree_test,mean(RSquared)];
end
...
figure
plot(MSE_Values(:,1),MSE_Values(:,4));
xlabel("Data %");
ylabel("R Squared");
title("R Squared Vs Data %");
