...
%****Dropping Columns after PCA Analysis*****
airport_dataset.ParkingFacilities=[];
airport_dataset.ParkingFacilities_valueForMoney_=[];
airport_dataset.AvailabilityOfBanks_ATM_moneyChanging=[];
airport_dataset.EaseOfMakingConnections=[];
airport_dataset.Business_executiveLounges=[];
airport_dataset.AvailabilityOfBaggageCarts=[];
...
for perc=1:100
    ...
    features_lim=score(1:split_amnt,:);
    target_lim=target(1:split_amnt,:);

    %Applying K-Folds Validation
    
    nKFolds=round(0.1*split_amnt);
    maxKFolds=500;
    ...
    parfor i=1:nKFolds
        ...
        % Decision Tree
        %=========================
        ctree=fitctree(training_features,training_target,"MaxNumSplits",10);
        
        predictions_train=predict(ctree,training_features);
        predictions_test=predict(ctree,testing_features);
    ...
    csvwrite("SystematicGridSearchResults.csv",[perc,RMSE_decisiontree_train,RMSE_decisiontree_test]);
end
...
xlabel("Data %");
ylabel("RMSE");
title("Learning Curve: Decission Tree");
